...
First implemented slice: Work items
...
`src/lib/application/workspace-api.ts` defines UI-facing query and command types. Callers depend on this module instead of importing work-item mutation adapters directly.

The first `WorkspaceApplicationApi` slice exposes:

- `workItems.list()`
- `workItems.create(command)`
- `workItems.updateStatus(command)`

Each command returns the saved work item and the authoritative workflow-risk reconciliation result from the same transaction.
...
For work items, direct browser insert, update, and delete privileges are revoked. Reads remain protected by existing workspace RLS.

### Atomic database commands

The migration adds two authenticated `security definer` functions:

- `command_create_workflow_task`
- `command_update_workflow_task_status`

Each function explicitly verifies that `auth.uid()` owns the workspace, validates its input, locks the affected record, performs the work-item change, reconciles risk signals and Workflow Health, and writes the durable Activity entry before committing.
...
The status command additionally validates the expected current status and rejects no-op status changes.

### Optimistic concurrency

Status updates include `expectedStatus`. If another session changes the work item first, the command returns a conflict instead of overwriting newer data.
...
| Client records | workspace records | create/update record | update + risk reconciliation + Activity | Pending migration to application command |
...
| Activity | workspace history | direct inserts from legacy flows | command-owned audit writes | Work-item audit implemented; other flows pending |

## Assistant Eligibility

The work-item commands are structurally suitable for a future protected assistant tool, but they are not exposed to an assistant yet. Assistant enablement also requires:
...
1. Move client-record update plus reconciliation and Activity into one command.
2. Move proposal and invoice create/update flows behind the application facade; retain their existing atomic recommendation RPCs.
3. Move handoff-note creation and audit into one command.
4. Move risk review status changes behind a command that performs reconciliation in the same transaction.
